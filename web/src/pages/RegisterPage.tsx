    ...
    for (let i = 0; i < entryLen; i++) {
      ...
      for (let j = 0; j < activeColIds.length; j++) {
        ...
        if (val === 'true') n = 1;
          ...
          // Strip currency symbols and commas, then check if result is a clean number
          const cleaned = val.replace(/[₹$,]/g, '').trim();
          // Only parse if the cleaned value is purely numeric (digits, optional decimal, optional leading minus)
          if (/^-?\d+(\.\d+)?$/.test(cleaned)) {
